...
interface TeacherQuoteBannerProps {
...
export function TeacherQuoteBanner({
  icon,
  ...
  quote,
  className,
}: TeacherQuoteBannerProps) {
  return (
    <div
      className={cn(
        "relative overflow-hidden rounded-2xl border border-blue-100 dark:border-stone-800 bg-[#F0F7FF] dark:bg-[#12161f] p-5 shadow-xs flex flex-col md:flex-row items-center justify-between gap-4 select-none mt-8",
        className
      )}
    >
      {/* Background Mountain Art Silhouette in Soft Cyan/Blue */}
      <div className="absolute right-0 bottom-0 top-0 w-80 md:w-96 pointer-events-none opacity-45 dark:opacity-20 flex items-end justify-end">
        <svg
          viewBox="0 0 400 120"
          fill="none"
          xmlns="http://www.w3.org/2000/svg"
          className="w-full h-full text-[#60A5FA] dark:text-blue-500"
        >
          {/* Mountain Ridge 1 */}
          <polygon
      ...
      <div className="flex items-center gap-4 relative z-10">
        ...
        <div>
          ...
          <p className="font-sans text-xs text-[#64748B] dark:text-stone-400 mt-0.5 font-medium">
            {subtitle}
          </p>
        </div>
      </div>
