...
export function Topbar({
  ...
  const effectiveUserName =
    userName === "Genevieve Laurent" && role === "SUPER_ADMIN"
      ? "Mr. Rajesh Pillai"
      : userName;
  const effectiveUserTitle =
    userTitle === "SCHOLAR • FORM VI (GRADE 12-IB) • ROSEY MANOR" && role === "SUPER_ADMIN"
  ...
  return (
    <>
      <header className="fixed top-0 left-0 md:left-sidebar-w right-0 h-16 bg-white/95 dark:bg-[#12161f]/95 backdrop-blur-md border-b border-stone-200/80 dark:border-stone-800 z-40 flex items-center justify-between px-4 md:px-6 shadow-[0_1px_6px_rgba(0,0,0,0.02)] transition-colors duration-200">
        {/* Left: Institution / Cluster Context */}
          ...
          {role === "SUPER_ADMIN" ? (
            <div className="flex items-center gap-2.5">
              <div className="flex items-center gap-2">
                ...
                <span className="font-sans text-sm font-semibold text-slate-900 dark:text-slate-100 tracking-tight">
                  India Central Cluster
                </span>
              </div>
              <span className="text-[11px] font-semibold text-emerald-700 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-950/50 border border-emerald-200/60 dark:border-emerald-800/50 px-2 py-0.5 rounded-full">
                Online
              </span>
            </div>
          ) : (
        ...
        <div className="flex items-center gap-2 md:gap-3 shrink-0">
          ...
          <div className="relative hidden lg:flex items-center">
            ...
            <input
              type="text"
              placeholder={role === "SUPER_ADMIN" ? "Search students, schools, records..." : "Search students, ledgers, records (⌘K)..."}
              className="w-56 xl:w-80 h-9 pl-9 pr-4 bg-stone-100/80 dark:bg-stone-800/80 border border-stone-200/60 dark:border-stone-700/80 rounded-xl text-xs text-stone-900 dark:text-stone-100 placeholder:text-stone-400 dark:placeholder:text-stone-500 focus:outline-none focus:bg-white dark:focus:bg-stone-800 focus:ring-1 focus:ring-blue-500/30 transition-all font-sans"
            />
          </div>
            ...
            {isUserMenuOpen && (
              <div className="absolute right-0 mt-2 w-56 bg-white dark:bg-[#1a202c] rounded-2xl border border-stone-200 dark:border-stone-700 shadow-lg py-2 z-50 animate-in fade-in slide-in-from-top-1 duration-150">
                <div className="px-4 py-2 border-b border-stone-100 dark:border-stone-800">
                  <p className="font-sans text-xs font-bold text-stone-900 dark:text-stone-100 truncate">{effectiveUserName}</p>
                  <p className="font-sans text-[10px] text-stone-500 dark:text-stone-400 font-medium truncate mt-0.5 uppercase tracking-wider">
                    {role === "SUPER_ADMIN" ? "Platform Lead & Super Admin" : `${role} • ${campusName}`}
                  </p>
                </div>
