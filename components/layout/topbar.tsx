...
export function Topbar({
  ...
  const displayOrgName = currentOrganization?.name || "Educational Organization";
  const displaySchoolName = allSchoolsMode
    ? "All Schools (Consolidated)"
    : currentSchool?.name || currentSchool?.legal_name || schoolName || "School Administration";
  const displayCampusCode = allSchoolsMode
    ? "MULTI-CAMPUS"
    : currentSchool?.school_code || campusName || "MAIN CAMPUS";
                ...
                {schools && schools.length > 0 ? (
                  schools.map((s) => (
                    <button
                      ...
                      type="button"
                      onClick={() => {
                        switchSchool(s.school_id);
                        setIsSchoolDropdownOpen(false);
                      }}
                      className={`w-full flex items-center justify-between px-2.5 py-2 rounded-lg text-xs hover:bg-slate-100 dark:hover:bg-white/5 text-left transition-colors ${
                        !allSchoolsMode && currentSchool?.id === s.school_id
                    ...
                    >
                      <div>
                        <div className="font-semibold text-slate-900 dark:text-slate-100">
                          {s.school?.name || s.school?.legal_name || "School Campus"}
                        </div>
                        <div className="text-[10px] text-slate-500">
                          {s.school?.school_code || "CAMPUS"} {s.school?.city ? `• ${s.school.city}` : ""}
                        </div>
                      </div>
                      {!allSchoolsMode && currentSchool?.id === s.school_id && (
                        <Check className="w-3.5 h-3.5 text-blue-600 shrink-0" />
                  ...
                  <button
                    type="button"
                    className="w-full flex items-center justify-between px-2.5 py-2 rounded-lg text-xs bg-blue-50 dark:bg-blue-950/40 text-blue-900 dark:text-blue-200 font-semibold text-left"
                  >
                    <div>
                      <div className="font-semibold">{currentSchool.name || currentSchool.legal_name}</div>
                      <div className="text-[10px] text-slate-500">{currentSchool.school_code || "CAMPUS"}</div>
                    </div>
                    <Check className="w-3.5 h-3.5 text-blue-600 shrink-0" />
                  </button>
                ) : (
                  <div className="px-2.5 py-2 text-xs text-slate-400">No schools registered</div>
                )}
