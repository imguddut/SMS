...
export async function runUnitTests() {
  console.log("\n========================================================");
  console.log("▶ RUNNING UNIT TESTS (Utils, Roles & Financial Math)");
  console.log("========================================================");
  ...
  // 3. Role Home Route Mapping Tests
  ...
  // 4. Financial Math Precision & Integrity Test
  try {
    const items = [
...
if (process.argv[1]?.endsWith("utils.test.ts") || process.argv[1]?.endsWith("run.ts")) {
  runUnitTests().then(() => {
    const summary = globalReporter.getSummary();
    console.log(`\nUnit Tests Completed: ${summary.passed}/${summary.total} Passed (${summary.passRatePercent}%)`);
  });
}
